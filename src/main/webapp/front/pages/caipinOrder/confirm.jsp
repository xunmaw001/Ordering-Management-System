...
<div id="app">
    ...
    <!-- 标题 -->
			...
			<h3 class="common_title">确认配送地址</h3>
			...
			<h3 class="common_title">配送时间:<input type="text" class="layui-input" id="yuyuepeisongTime" placeholder="请选择配送时间" style="width: 300px"></h3>
            ...
            <div class="common_list_con clearfix">
                ...
                <ul class="goods_list_td clearfix"  v-for="(item,index) in dataList" v-bind:key="index">
                    ...
                    <li class="col02"><img :src="item.caipinPhoto"></li><!-- style="width: 100px;height: 100px;object-fit: cover;"-->
                    <li class="col03">{{item.caipinName}}</li>
			...
			<div class="common_list_con clearfix">
				<div class="settle_con" v-if="this.caipinOrderPaymentTypes == 1">
				...
				<div class="settle_con" v-if="this.caipinOrderPaymentTypes == 2">
		...
		<script>
			var vue = new Vue({
				...
				methods: {
					...
					,payClick() {
						...
						if (!index) {
                            layui.layer.msg('请选择配送地址', {
                                time: 2000,
                                icon: 5
                            });
                            return false;
                        }
                        let yuyuepeisongTime = layui.jquery("#yuyuepeisongTime").val();
						if(yuyuepeisongTime == null || yuyuepeisongTime ==""){
                            layui.layer.msg('请选择要配送的时间', {
                                time: 2000,
                                icon: 5
                            });
                            return false;
						...
						let data={
                            ...
                            caipins:localStorage.getItem('caipins'),
                            yonghuId: localStorage.getItem('userid'),
                            yuyuepeisongTime: yuyuepeisongTime,
                            caipinOrderPaymentTypes: vue.caipinOrderPaymentTypes,
						}
                        // 获取商品详情信息
                        layui.http.request(`caipinOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../caipinOrder/list.jsp';
							}
                       		 window.location.href='../caipinOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate'], function() {
                ...
                // 获取菜品购买的清单列表
				var caipins = localStorage.getItem('caipins');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(caipins);
