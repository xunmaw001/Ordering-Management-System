                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <i @click="deleteCaipinCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                                    <div class="ng-prslist-img" @click="jumpCheck('../caipin/detail.jsp?id='+item.caipinId , item.caipinDelete == null?'':item.caipinDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" >
                                        <a href="#" class="ng-prsliik">
                                            <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                                <img class="swiper-item" :src="item.caipinPhoto?item.caipinPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                            </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jumpCheck('../caipin/detail.jsp?id='+item.caipinId , item.caipinDelete == null?'':item.caipinDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" ><span>{{item.caipinName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.caipinNewMoney"><span style="color: red;">{{item.caipinNewMoney}} RMB</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    ...
    // 当前表的字典表
        ...
        function caipinCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=caipin_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.caipinCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function pageList() {
            ...
            //收藏只能查类型为1的
            param['caipinCollectionTypes'] = 1;

            // 获取列表数据
            http.request('caipinCollection/page', 'get', param, function(res) {
