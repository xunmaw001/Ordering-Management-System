                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 caipinOrderUuidNumberDiv">
                                        <label>订单号</label>
                                        <input style="width: 450px" id="caipinOrderUuidNumber" name="caipinOrderUuidNumber" class="form-control"
                                               placeholder="订单号">
                                    </div>
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6 caipinOrderTruePriceDiv">
                                        <label>实付价格</label>
                                        <input style="width: 450px" id="caipinOrderTruePrice" name="caipinOrderTruePrice" class="form-control"
                                               onchange="caipinOrderTruePriceChickValue(this)" placeholder="实付价格">
                                    </div>
                                    <div class="form-group col-md-6 caipinOrderCourierNameDiv">
                                        <label>送餐人姓名</label>
                                        <input style="width: 450px" id="caipinOrderCourierName" name="caipinOrderCourierName" class="form-control"
                                               placeholder="送餐人姓名">
                                    </div>
                                    <div class="form-group col-md-6 caipinOrderCourierNumberDiv">
                                        <label>送餐人手机号</label>
                                        <input style="width: 450px" id="caipinOrderCourierNumber" name="caipinOrderCourierNumber" class="form-control"
                                               placeholder="送餐人手机号">
                                    </div>
                                    <div class="form-group col-md-6 caipinOrderTypesDiv">
                                        <label>订单类型</label>
                                        <select style="width: 450px" id="caipinOrderTypesSelect" name="caipinOrderTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6 caipinOrderPaymentTypesDiv">
                                        <label>支付类型</label>
                                        <select style="width: 450px" id="caipinOrderPaymentTypesSelect" name="caipinOrderPaymentTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6 yuyuepeisongTimeDiv">
                                        <label>预约配送时间</label>
                                        <input style="width: 450px" id="yuyuepeisongTime-input" name="yuyuepeisongTime" type="text" class="form-control layui-input">
                                    </div>
...
<script>
    ...
    // 下拉框数组
        <!-- 当前表的下拉框数组 -->
    var caipinOrderTypesOptions = [];
    var caipinOrderPaymentTypesOptions = [];
        <!-- 级联表的下拉框数组 -->
    var addressOptions = [];
    var caipinOptions = [];
    var yonghuOptions = [];
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            let data = {};
            getContent();
            if(window.sessionStorage.getItem('role') != '配送地址'){//当前登录用户不为这个
                if($("#addressId") !=null){
    ...
    // 查询列表
        ...
        function caipinOrderTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=caipin_order_types", "GET", {}, (res) => {
                if(res.code == 0){
                    caipinOrderTypesOptions = res.data.list;
                }
            });
        }
        function caipinOrderPaymentTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=caipin_order_payment_types", "GET", {}, (res) => {
                if(res.code == 0){
                    caipinOrderPaymentTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationCaipinordertypesSelect(){
            var caipinOrderTypesSelect = document.getElementById('caipinOrderTypesSelect');
            if(caipinOrderTypesSelect != null && caipinOrderTypesOptions != null  && caipinOrderTypesOptions.length > 0 ){
                for (var i = 0; i < caipinOrderTypesOptions.length; i++) {
                    caipinOrderTypesSelect.add(new Option(caipinOrderTypesOptions[i].indexName,caipinOrderTypesOptions[i].codeIndex));
                }
            }
        }
        function initializationCaipinorderpaymenttypesSelect(){
            var caipinOrderPaymentTypesSelect = document.getElementById('caipinOrderPaymentTypesSelect');
            if(caipinOrderPaymentTypesSelect != null && caipinOrderPaymentTypesOptions != null  && caipinOrderPaymentTypesOptions.length > 0 ){
                for (var i = 0; i < caipinOrderPaymentTypesOptions.length; i++) {
                    caipinOrderPaymentTypesSelect.add(new Option(caipinOrderPaymentTypesOptions[i].indexName,caipinOrderPaymentTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(caipinOrderTypesSelect != null && caipinOrderTypesOptions != null  && caipinOrderTypesOptions.length > 0 ) {
            for (var i = 0; i < caipinOrderTypesOptions.length; i++) {
                if (caipinOrderTypesOptions[i].codeIndex == ruleForm.caipinOrderTypes) {//下拉框value对比,如果一致就赋值汉字
                        caipinOrderTypesSelect.options[i].selected = true;
                }
            }
        }
        ...
        if(caipinOrderPaymentTypesSelect != null && caipinOrderPaymentTypesOptions != null  && caipinOrderPaymentTypesOptions.length > 0 ) {
            for (var i = 0; i < caipinOrderPaymentTypesOptions.length; i++) {
                if (caipinOrderPaymentTypesOptions[i].codeIndex == ruleForm.caipinOrderPaymentTypes) {//下拉框value对比,如果一致就赋值汉字
                        caipinOrderPaymentTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    //数字检查
        ...
        function caipinOrderTruePriceChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                caipinOrderUuidNumber: "required",
                addressId: "required",
                caipinId: "required",
                yonghuId: "required",
                buyNumber: "required",
                caipinOrderTruePrice: "required",
                caipinOrderCourierName: "required",
                caipinOrderCourierNumber: "required",
                caipinOrderTypes: "required",
                caipinOrderPaymentTypes: "required",
                yuyuepeisongTime: "required",
            },
            messages: {
                caipinOrderUuidNumber: "订单号不能为空",
                addressId: "配送地址不能为空",
                caipinId: "菜品不能为空",
                yonghuId: "用户不能为空",
                buyNumber: "购买数量不能为空",
                caipinOrderTruePrice: "实付价格不能为空",
                caipinOrderCourierName: "送餐人姓名不能为空",
                caipinOrderCourierNumber: "送餐人手机号不能为空",
                caipinOrderTypes: "订单类型不能为空",
                caipinOrderPaymentTypes: "支付类型不能为空",
                yuyuepeisongTime: "预约配送时间不能为空",
            }
        }).form();
    }
    ...
    function getDetails() {
        ...
        if (addcaipinOrder != null && addcaipinOrder != "" && addcaipinOrder != "null") {
            //注册表单验证
            $(validform());
            $("#caipinOrderUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#caipinOrderUuidNumber").val(ruleForm.caipinOrderUuidNumber);
        $("#addressId").val(ruleForm.addressId);
        $("#caipinId").val(ruleForm.caipinId);
        $("#yonghuId").val(ruleForm.yonghuId);
        $("#buyNumber").val(ruleForm.buyNumber);
        $("#caipinOrderTruePrice").val(ruleForm.caipinOrderTruePrice);
        $("#caipinOrderCourierName").val(ruleForm.caipinOrderCourierName);
        $("#caipinOrderCourierNumber").val(ruleForm.caipinOrderCourierNumber);
        $("#yuyuepeisongTime-input").val(ruleForm.yuyuepeisongTime);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationCaipinordertypesSelect();
            initializationCaipinorderpaymenttypesSelect();
            <!--  初始化级联表的下拉框  -->
