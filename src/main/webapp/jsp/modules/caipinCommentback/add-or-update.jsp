                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 caipinCommentbackPingfenNumberDiv">
                                        <label>评分</label>
                                        <input style="width: 450px" id="caipinCommentbackPingfenNumber" name="caipinCommentbackPingfenNumber" class="form-control"
                                               onchange="caipinCommentbackPingfenNumberChickValue(this)"  placeholder="评分">
                                    </div>

                                    <div class="form-group col-md-6 caipinCommentbackTextDiv">
                                        <label>评价内容</label>
                                        <textarea style="width: 450px"  id="caipinCommentbackText" name="caipinCommentbackText" class="form-control"placeholder="评价内容"></textarea>
                                    </div>
...
<script>
    ...
    var tableName = "caipinCommentback";
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("caipinCommentback/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addcaipinCommentback');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function caipinCommentbackPingfenNumberChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                caipinId: "required",
                yonghuId: "required",
                caipinCommentbackPingfenNumber: "required",
                caipinCommentbackText: "required",
                replyText: "required",
            },
            messages: {
                caipinId: "菜品不能为空",
                yonghuId: "用户不能为空",
                caipinCommentbackPingfenNumber: "评分不能为空",
                caipinCommentbackText: "评价内容不能为空",
                replyText: "回复内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addcaipinCommentback = window.sessionStorage.getItem("addcaipinCommentback");
        if (addcaipinCommentback != null && addcaipinCommentback != "" && addcaipinCommentback != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("caipinCommentback/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#caipinCommentbackPingfenNumber").val(ruleForm.caipinCommentbackPingfenNumber);
        $("#caipinCommentbackText").val(ruleForm.caipinCommentbackText);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            $('#caipinCommentbackText').attr('readonly', 'readonly');
