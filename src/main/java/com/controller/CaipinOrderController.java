...
//                            caipinOrderEntity.setAddressId(Integer.valueOf(data.get(0)));   //配送地址 要改的
...
//                            caipinOrderEntity.setCaipinOrderCourierName(data.get(0));                    //送餐人姓名 要改的
//                            caipinOrderEntity.setCaipinOrderCourierNumber(data.get(0));                    //送餐人手机号 要改的
...
//                            caipinOrderEntity.setYuyuepeisongTime(new Date(data.get(0)));          //预约配送时间 要改的
...
//                            caipinOrderEntity.setCreateTime(date);//时间
    ...
    */
    ...
    public R add(@RequestParam Map<String, Object> params, HttpServletRequest request) throws ParseException {
        ...
        String yuyuepeisongTime = String.valueOf(params.get("yuyuepeisongTime"));//预约配送时间
        if(yuyuepeisongTime == null || "".equals(yuyuepeisongTime)  || "null".equals(yuyuepeisongTime)){
            return R.error("预约配送时间不能为空");
        }
        ...
        for (Map<String, Object> map : caipins) {
           //取值
            ...
            caipinOrderEntity.setAddressId(addressId);//配送地址
            ...
            caipinOrderEntity.setYuyuepeisongTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(yuyuepeisongTime));//预约配送时间 ？？？？？？
    ...
    */
    ...
    public R commentback(Integer id, String commentbackText, Integer caipinCommentbackPingfenNumber, HttpServletRequest request){
        ...
        //查询这个菜品的评价总分
        List<CaipinCommentbackEntity> caipinCommentbackEntities = caipinCommentbackService.selectList(new EntityWrapper<CaipinCommentbackEntity>().eq("caipin_id", caipinId));

        Double zong=0.0;
        for(CaipinCommentbackEntity c:caipinCommentbackEntities){
            zong=c.getCaipinCommentbackPingfenNumber()+zong;
        }

        CaipinEntity caipinEntity = new CaipinEntity();
        caipinEntity.setId(caipinId);
        caipinEntity.setCaipinPingfen(zong/caipinCommentbackEntities.size());
        caipinService.updateById(caipinEntity);
